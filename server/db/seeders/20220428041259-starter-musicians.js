...
module.exports = {
  up: async (queryInterface, Sequelize) => {
   ...
   for (let i = 0; i < bandMusicians.length; i++) {
     let band = bandMusicians[i];
     const { name, musicians } = band;

     const singleBand = await Band.findOne({
       where: {
         name: name
       }
     })
      // const { musicians: { firstName }, musicians: { lastName } } = band

      for (let i = 0; i < musicians.length; i++) {
        const singleMusician = musicians[i]
        await singleBand.createMusician(singleMusician)
      }
